You're going to supervise sub-agents working through items identified in either:
- @rel-04/tasks/order.md, or
- @rel-04/fixes/order.md
...
Each task/fix file indicates what model to use when running the sub-agent. It also provides the sub-agent with a list of documents to pre-read and the work instructions they will need.
...
1. Read the next incomplete item from the appropriate order.md (tasks/ or fixes/)
2. Launch the appropriate sub-agent (model specified in task/fix file) with the task instructions
...
or
```
Fix: item-name.md - Short description
