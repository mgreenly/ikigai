...
**Action:** Executes `make coverage` and uses sub-agents with the coverage persona to fix any coverage issues found. Sub-agents run sequentially (one per file) and continue until all issues are resolved.
...
### 1. Initial Check
Run `make coverage`. If it passes, report success and stop.
If it fails, identify all files with coverage gaps.

### 2. Fix Issues With Sub-Agents

**Scope:** One sub-agent per file. Each sub-agent fixes ALL coverage gaps in its assigned file.
...
- All coverage gaps found in that file
...
### 3. Verify Results
After ALL sub-agents have completed, re-run `make coverage`.
