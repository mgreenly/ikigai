...
The wiki is ikigenba's knowledge base: an MCP service that *compiles* ingested text into durable per-subject pages and answers questions from them. Each ingest breaks text into the **subjects** it concerns and the **claims** it makes, then writes or updates each subject's page, so knowledge **compounds** — the LLM does the upkeep humans abandon; the user curates what goes in and asks. The owner can **merge** two names for the same thing into one page. Around that core the wiki records **every LLM call** and lets the user steer the ingest queue (list, abort, re-run); reads are always served and never wait on the slow, strictly-serial ingest work. Every call site is configured **independently**, and the developer can take a call site **off-line and evaluate** it against a human-blessed gold dataset to choose what runs in production on evidence rather than guesswork.
...
- **Record and inspect every LLM call** — for extract, compile, and ask, store the exact request, exact response, parameters actually used (provider, model, generation settings), stage/job/attempt, error flag, and token usage; each retry is its own record; kept **permanently** and **accumulating across re-runs**; listable, filterable by job/stage/time, paged. ask's calls are recorded, unattached to a job.
- **See ingest progress** — by handle, report a job's state and, when finished, the subjects it produced.
- **Ask** — agentically extract the question's subjects, resolve each by **exact normalized name**, read the resolved pages, and synthesize a **cited** answer from **wiki content only**; use every subject that resolves (best-effort partial); when none resolve, answer that the wiki holds nothing.
...
- **Configure each call site independently** — `extract`, `compile`, and `ask` each get their own model, thinking level, and generation settings, with no two forced to match and no global; the configuration is inspectable. (Evaluation adds one eval-only call site, the **judge**, configured the same way.)
...
- **The production LLM call sites are `extract`, `compile`, `ask`** — each named, each configured independently. Evaluation adds the **judge**, which exists only inside an evaluation run.
...
- **Every LLM call is on the record** — request, response, parameters, error, tokens, every retry — kept forever and accumulating across re-runs, so you can compare a fixed prompt against the broken one.
...
- **Ask works like an agent and is honest.** It answers from the pages of the subjects your question names, cited by readable name; answers from every one it has even if another was never ingested; says "nothing here" plainly when none resolve; never fabricates from general knowledge; never changes anything.
- **Each call site runs its own model,** independently configured and inspectable — no single global model.
...
- For any LLM call (extract/compile/ask, each retry) I see the exact request, response, parameters (provider, model, temperature, reasoning), error flag, and tokens.
...
- I ask a question naming a subject the wiki has, and get a cited answer synthesized from its page, drawn only from ingested content.
- I ask a question naming several subjects; the wiki answers from every one it has and does not fail because one was never ingested.
- Asking when none of the named subjects exist returns an explicit "nothing here," not a fabricated answer.
...
- I configure `extract`, `compile`, and `ask` with different models/settings at once, none forced to share, and confirm which model runs behind each.
