# Phase 63 — The landing page: `internal/web` + `landingHandler` wired at `GET /{$}`

*Realizes design Decision 39 (the session-gated human web surface). New package
`internal/web`; touches the composition root `cmd/wiki/main.go`. No migration,
no DB, no LLM. Depends only on the appkit `Router` interface (`HandleFunc`,
`Service()`, `Version()`) already used by `buildSpec`.*

wiki gains its first HTML web page — a Carbon-styled landing page at the bare
mount root showing the service **name** and **version**. It is added beside the
unchanged MCP/`/health`/PRM surfaces and is mounted **ungated in-process** (nginx
is the gate, added in Phase 64).

In a new **`wiki/internal/web`** package:
...
- Embed the page assets via `//go:embed`, mirroring the dashboard's `ui/`
  ...
  `embed.FS`): a `landing.tmpl` (Go `html/template`) plus a `static/` dir
  holding `tokens.css` and the woff2 font file(s) (IBM Plex Sans/Mono, Space
  Grotesk), **copied** from the repo-root Carbon source (`design/tokens.css` +
...
- `LandingHandler(service, version string) http.HandlerFunc` — renders
  ...
  function** of name+version: no DB, no LLM, no identity header read. The
  template references the embedded `tokens.css`/fonts via a sibling static route.
- A static asset handler (e.g. `StaticHandler() http.Handler` over the embedded
  `static/` sub-FS) serving `tokens.css` + fonts with correct content-types.
...
In **`cmd/wiki/main.go`** (`spec.Handlers`, beside the existing `POST /mcp`
wiring), mount **ungated** — do **not** wrap in `rt.RequireIdentity`:
...
wiki`) and these ids are covered by clearly-named tests in
`wiki/internal/web/web_test.go` (`package web`), driving the handlers via
`net/http/httptest` (no DB, no LLM, no live call):
...
- **R-LAND-CARB** — the embedded `embed.FS` contains `tokens.css` and the
  woff2 font(s), the served page references `tokens.css`, and a request for the
