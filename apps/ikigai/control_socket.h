...
res_t ik_control_socket_init(TALLOC_CTX *ctx, ik_paths_t *paths, ik_control_socket_t **out);
...
// Asserts socket != NULL
void ik_control_socket_add_to_fd_sets(ik_control_socket_t *socket, fd_set *read_fds, int *max_fd);

// Check if listen fd is ready in fd_set (after select())
// Returns true if listen_fd is set in read_fds
// Asserts socket != NULL
bool ik_control_socket_listen_ready(ik_control_socket_t *socket, fd_set *read_fds);

// Check if client fd is ready in fd_set (after select())
// Returns true if client is connected and client_fd is set in read_fds
// Asserts socket != NULL
bool ik_control_socket_client_ready(ik_control_socket_t *socket, fd_set *read_fds);
...
res_t ik_control_socket_handle_client(ik_control_socket_t *socket, ik_repl_ctx_t *repl);
