...
res_t ik_cmd_clear(void *ctx, ik_repl_ctx_t *repl, const char *args)
{
    ...
    // Clear skill catalog entries
    if (repl->current->skillset_catalog != NULL) {  // LCOV_EXCL_BR_LINE
        for (size_t i = 0; i < repl->current->skillset_catalog_count; i++) {
            talloc_free(repl->current->skillset_catalog[i]);
        }
        repl->current->skillset_catalog_count = 0;
    }
