...
char *
expand_tilde (TALLOC_CTX *ctx, const char *path)
{
  if (!path || path[0] != '~')
    {
...
static ik_result_t
create_default_config (TALLOC_CTX *ctx, const char *path)
{
  ...
  struct stat st;
  if (stat (dir, &st) != 0)
    {				// LCOV_EXCL_BR_LINE - directory exists case difficult to test
...
ik_result_t
ik_cfg_load (TALLOC_CTX *ctx, const char *path)
{
  if (!path)
    {
  ...
  if (json_typeof (api_key) != JSON_STRING)
    {
      json_decref (root);
  ...
  if (json_typeof (port) != JSON_INTEGER)
    {
      json_decref (root);
  ...
  json_int_t port_raw = json_integer_value (port);
