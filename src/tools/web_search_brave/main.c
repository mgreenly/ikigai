...
int32_t main(int32_t argc, char **argv)
{
    ...
    if (argc == 2 && strcmp(argv[1], "--schema") == 0) {
        printf("{\n");
        printf("  \"name\": \"web_search_brave\",\n");
        printf("  \"description\": \"Search the web using Brave Search API and use the results to inform responses. Provides up-to-date information for current events and recent data. Returns search result information formatted as search result blocks, including links as markdown hyperlinks.\",\n");
        printf("  \"parameters\": {\n");
        printf("    \"type\": \"object\",\n");
        printf("    \"properties\": {\n");
        printf("      \"query\": {\n");
        ...
        printf("      \"count\": {\n");
        printf("        \"type\": \"integer\",\n");
        printf("        \"description\": \"Number of results to return (1-20)\",\n");
        printf("        \"minimum\": 1,\n");
        printf("        \"maximum\": 20,\n");
        printf("        \"default\": 10\n");
        printf("      },\n");
        printf("      \"offset\": {\n");
        printf("        \"type\": \"integer\",\n");
        printf("        \"description\": \"Result offset for pagination\",\n");
        printf("        \"minimum\": 0,\n");
        printf("        \"default\": 0\n");
        printf("      },\n");
    ...
    if (curl == NULL) {
        printf("{\"success\": false, \"error\": \"Failed to initialize HTTP client\", \"error_code\": \"NETWORK_ERROR\"}\n");
        talloc_free(ctx);
        return 0;
    }

    char *url = talloc_asprintf(ctx, "https://api.search.brave.com/res/v1/web/search?q=%s&count=%" PRId32 "&offset=%" PRId32, query_str, count, offset);
    if (url == NULL) {
    ...
    talloc_free(url);
    url = talloc_asprintf(ctx, "https://api.search.brave.com/res/v1/web/search?q=%s&count=%" PRId32 "&offset=%" PRId32, escaped_query, count, offset);
    ...
    if (http_code == 401 || http_code == 403) {
        printf("{\"success\": false, \"error\": \"API key is invalid or unauthorized\", \"error_code\": \"AUTH_INVALID\"}\n");
        talloc_free(ctx);
        return 0;
    }

    if (http_code == 429) {
        printf("{\"success\": false, \"error\": \"Rate limit exceeded. You've used your free search quota (2,000/month).\", \"error_code\": \"RATE_LIMIT\"}\n");
        talloc_free(ctx);
        return 0;
    }
    ...
    if (web == NULL) {
        printf("{\"success\": false, \"error\": \"Missing web results in API response\", \"error_code\": \"API_ERROR\"}\n");
        talloc_free(ctx);
        return 0;
    }
    ...
    yyjson_arr_foreach(results, idx, max, item) {
        ...
        if (allowed_domains != NULL && yyjson_is_arr(allowed_domains)) {
            int32_t matches = 0;
            size_t aidx, amax;
            yyjson_val *domain;
            yyjson_arr_foreach(allowed_domains, aidx, amax, domain) {
        ...
        if (blocked_domains != NULL && yyjson_is_arr(blocked_domains)) {
            int32_t blocked = 0;
            size_t bidx, bmax;
            yyjson_val *domain;
            yyjson_arr_foreach(blocked_domains, bidx, bmax, domain) {
                if (yyjson_is_str(domain)) {
                    if (domain_matches(url_str, yyjson_get_str(domain))) {
                        blocked = 1;
                        break;
