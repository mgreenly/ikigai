...
res_t ik_scrollback_layer_create(TALLOC_CTX *ctx, const char *name, ik_scrollback_t *scrollback,
...
res_t ik_separator_layer_create(TALLOC_CTX *ctx, const char *name, bool *visible_ref, ik_layer_t **layer_out);
...
// visible_ref and text_ref/len_ref are borrowed pointers that must remain valid
...
res_t ik_spinner_layer_create(TALLOC_CTX *ctx, const char *name, ik_spinner_state_t *state, ik_layer_t **layer_out);
