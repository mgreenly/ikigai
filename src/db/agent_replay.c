                                     ...
                                     size_t range_msg_count)
{
    ...
    if (range_msg_count == 0) {
        return OK(NULL);
    }

    assert(range_msgs != NULL);  // LCOV_EXCL_BR_LINE
...
void ik_agent_filter_interrupted_turns(ik_replay_context_t *replay_ctx)
{
    ...
    bool in_interrupted_turn = false;
    ...
    (void)in_interrupted_turn;  // Silence unused variable warning
}
                              ...
                              ik_replay_context_t **ctx_out)
{
    ...
    for (size_t i = 0; i < range_count; i++) {
        ...
        res = ik_agent_append_range_messages(replay_ctx, range_msgs, range_msg_count);
        if (is_err(&res)) { // LCOV_EXCL_BR_LINE
            talloc_steal(ctx, res.err); // LCOV_EXCL_LINE
            talloc_free(tmp); // LCOV_EXCL_LINE
            return res; // LCOV_EXCL_LINE
    ...
    ik_agent_filter_interrupted_turns(replay_ctx);
