...
res_t ik_repl_handle_page_up_action(ik_repl_ctx_t *repl)
{
    ...
    size_t new_offset = repl->viewport_offset + (size_t)repl->shared->term->screen_rows;
...
res_t ik_repl_handle_page_down_action(ik_repl_ctx_t *repl)
{
    ...
    if (repl->viewport_offset >= (size_t)repl->shared->term->screen_rows) {
        repl->viewport_offset -= (size_t)repl->shared->term->screen_rows;
    } else {
