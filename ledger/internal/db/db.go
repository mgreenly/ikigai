// Package db holds ledger's embedded migration set and a thin migrate helper for
...
// §B / C1) — this package keeps only what is app-side: the *.sql files (embedded
...
// Open opens ledger's SQLite database with the chassis pragmas (delegates to
// appkit/db so there is one Open implementation across every service).
func Open(dbPath string) (*sql.DB, error) {
	return db.Open(dbPath)
}

// Migrate applies ledger's embedded migrations against conn using appkit's
