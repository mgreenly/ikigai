...
Ledger's landing page gains a quiet **top-left Home link** that returns the
viewer to the dashboard landing page — the apex root `/`, which nginx routes to
the dashboard. This is ledger's own control, built entirely within ledger; nothing
...
- **`ledger/internal/web/web_test.go`** — add a genuinely-asserting test tagged
  `// R-HOME-4M6R` that drives `GET /` through the landing handler and asserts the
...
- R-HOME-4M6R — a test tagged `// R-HOME-4M6R` drives `GET /` through the landing handler and
