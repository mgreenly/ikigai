// Package db holds scripts' embedded migration set and a thin Open/Migrate
// helper for the domain tests. The SQLite handle and the forward-only migration
// runner + downgrade guard are the uniform chassis half and live in appkit/db —
// this package keeps only what is app-side: the *.sql files (embedded for
// Spec.Migrations).
package db
...
// Open opens scripts' SQLite database with the chassis pragmas (delegates to
// appkit/db so there is one Open implementation across every service).
func Open(dbPath string) (*sql.DB, error) {
	return db.Open(dbPath)
}

// Migrate applies scripts' embedded migrations against conn using appkit's
