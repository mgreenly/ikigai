...
Stand up scripts's human landing page: a new `scripts/internal/web` package with
a Carbon-styled handler showing the service name + running version, its own
embedded `tokens.css` + woff2 fonts, mounted ungated in-process at the exact root
`GET /{$}` through the existing `cmd/scripts/main.go` `registerRoutes` (the
`Handlers` hook; nginx is the gate — Phase 2).

**What gets built (the observable end state):**

- **`scripts/internal/web/`** — a new package:
...
- **`scripts/cmd/scripts/main.go`** — one line added to the **existing**
  `registerRoutes` function, beside the `POST /mcp` mount, ungated in-process:
  ...
  mount, the script Service construction (store, `python3`-exec runner,
  crash-recovery sweep), the consumer `Workers`, and the `Producer` outbox hook
  are untouched. (If `rt.HandleFunc` is not the exact appkit Router method name,
  use the Router's equivalent route-register call for `GET /{$}` — the contract
  is: the bare root, exact-match, served by the landing handler.)
...
- R-LAND-7Q3D — a test constructs `web.LandingHandler("scripts", v)` and drives a
  `GET /` through it (httptest), asserting status `200`.
- R-LAND-9R5F — that test asserts the rendered body contains the service name
  passed in (`"scripts"`).
- R-LAND-1S7G — a test passes a distinctive version (e.g. `"9.9.9-test"`) and
...
- R-LAND-3T9H — a test asserts the landing response's `Content-Type` is
  `text/html; charset=utf-8`.
- R-ROUT-8U2J — a test registers the handler on an `http.ServeMux` at
  `GET /{$}` (as the composition root wires it) and asserts `GET /` is dispatched
  to the landing handler (`200`, name+version body).
- R-ROUT-1V4K — a test registers a sibling `POST /mcp` on the same mux and asserts
  `{$}` does not shadow it (`POST /mcp` reaches the mcp/stub handler, not the
  landing handler).
- R-ROUT-3W6L — a test asserts a non-root unregistered path (e.g. `GET /nope`) is
  **not** captured by the `{$}` landing pattern (it does not return the landing
  page) — proving exact-match, not subtree.
- R-ASST-5X8M — a test drives `GET /static/tokens.css` through the handler and
  asserts `200` with a CSS content type (`text/css`).
- R-ASST-7Y1N — a test asserts the rendered landing HTML references scripts's own
  embedded `/static/` asset path and contains **no** cross-service/dashboard asset
  URL.
- R-ASST-9Z3P — a test drives `GET` for an embedded font (e.g.
