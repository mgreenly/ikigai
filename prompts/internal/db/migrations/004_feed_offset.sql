...
-- internal/db/migrations_feed_offset_test.go asserts it. agent's own migration
...
--
-- agent is now a CONSUMER (of cron's /feed) as well as a domain service. Its one
-- feed_offset row keys on the upstream source "cron". There is deliberately NO
-- dedup table: agent's run model is in-memory fire-and-run (event-triggering
