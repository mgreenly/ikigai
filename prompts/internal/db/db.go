// Package db holds prompts' embedded migration set and a thin Open/Migrate helper
...
// for Spec.Migrations) and the prompts_schema_test that asserts 002_prompts.sql lands
...
// Open opens prompts' SQLite database with the chassis pragmas (delegates to
// appkit/db so there is one Open implementation across every service).
func Open(dbPath string) (*sql.DB, error) {
	return db.Open(dbPath)
}

// Migrate applies prompts' embedded migrations against conn using appkit's
