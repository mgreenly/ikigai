...
LIFECYCLE
  1. ikigenba_prompts_session_create {prompt, config} -> {session_id, status:"idle"}
  2. ikigenba_prompts_session_run {session_id}        -> starts a run, returns immediately
  3. poll ikigenba_prompts_session_get {session_id}   -> watch last_run.status until it is
     succeeded | failed | cancelled
  4. ikigenba_prompts_session_output {session_id}     -> the run's narrated log
  5. ikigenba_prompts_session_fs_list / ikigenba_prompts_session_fs_read -> the files the agent wrote

OUTPUT FORMAT
- ikigenba_prompts_session_output returns the latest run's log as append-only stream-json:
...
- The *answer* is usually a file in the sandbox (e.g. report.md), not the
  agent's final message — read it with ikigenba_prompts_session_fs_read.

WORKED EXAMPLE
  ikigenba_prompts_session_create {"prompt":"Summarize X into report.md",
                        "config":{"model":"claude-sonnet-4-6","effort":"low"}}
    -> {"session_id":"S","status":"idle"}
  ikigenba_prompts_session_run {"session_id":"S"}
  ikigenba_prompts_session_get {"session_id":"S"}   # repeat until last_run.status=="succeeded"
  ikigenba_prompts_session_fs_list {"session_id":"S"}                    # -> report.md
  ikigenba_prompts_session_fs_read {"session_id":"S","path":"report.md"}
