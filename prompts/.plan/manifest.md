...
DEST root = `/mnt/projects/ikigai/agent/agent/internal/engine/`.
...
```
github.com/ai4mgreenly/ikigai-cli/internal/  ->  agent/internal/engine/
```
...
```
github.com/ai4mgreenly/ikigai-cli/internal/provider/anthropic -> agent/internal/engine/provider/anthropic
github.com/ai4mgreenly/ikigai-cli/internal/tools/bash         -> agent/internal/engine/tools/bash
github.com/ai4mgreenly/ikigai-cli/internal/tools/edit         -> agent/internal/engine/tools/edit
github.com/ai4mgreenly/ikigai-cli/internal/tools/glob         -> agent/internal/engine/tools/glob
github.com/ai4mgreenly/ikigai-cli/internal/tools/grep         -> agent/internal/engine/tools/grep
github.com/ai4mgreenly/ikigai-cli/internal/tools/read         -> agent/internal/engine/tools/read
github.com/ai4mgreenly/ikigai-cli/internal/tools/write        -> agent/internal/engine/tools/write
github.com/ai4mgreenly/ikigai-cli/internal/provider           -> agent/internal/engine/provider
github.com/ai4mgreenly/ikigai-cli/internal/tools              -> agent/internal/engine/tools
github.com/ai4mgreenly/ikigai-cli/internal/model              -> agent/internal/engine/model
github.com/ai4mgreenly/ikigai-cli/internal/schema             -> agent/internal/engine/schema
github.com/ai4mgreenly/ikigai-cli/internal/trace              -> agent/internal/engine/trace
github.com/ai4mgreenly/ikigai-cli/internal/wire               -> agent/internal/engine/wire
```

A single blanket sed of the common prefix
`github.com/ai4mgreenly/ikigai-cli/internal/` → `agent/internal/engine/` over
...
`go get` / no new `require` is needed for the engine.** `agent`'s `go.mod` keeps
...
Because `cmd/ikigai-cli/main.go` is the CLI and is not copied into agent, **the
google/openai imports never enter the agent build**. agent will write its own
small factory in its `cmd`/agent-bring-up code that calls only
`engine/provider/anthropic.New(...)`.

**Minimal edit for agent's new factory (Phase 2/3):** construct the client with
...
Note: agent may optionally prune the OpenAI/Google entries from
...
which becomes `agent/internal/engine/`.
...
## E. Ledger -> agent rename map

Chassis tree: `/mnt/projects/ikigai/agent/ledger/`. Module `ledger`.

### E.1 Global parameter changes

| item | ledger value | agent value |
|---|---|---|
| go module name | `ledger` | `agent` |
| cmd dir | `cmd/ledger/` | `cmd/agent/` |
| env prefix | `LEDGER_` | `AGENT_` |
| loopback port | `3002` | `3004` |
| nginx mount | `/srv/ledger/` | `/srv/agent/` |
| MCP tool prefix | `ledger_` | `agent_` |
| MCP server name | `Ledger` | `Agent` |
| sqlite db file | `ledger.db` | `agent.db` |
| install root | `/opt/ledger` | `/opt/agent` |
| systemd unit | `ledger` (`ledger.conf`, `ExecStart=ikigenba-launch ledger`) | `agent` |
| Makefile APP | `ledger` | `agent` |

Also: add `./agent` to `/mnt/projects/ikigai/agent/go.work` `use(...)` block —
**go.work currently lists crm/dashboard/eventplane/ledger/notify but NOT agent.**

### E.2 go.mod

`/mnt/projects/ikigai/agent/ledger/go.mod` line 1: `module ledger` ->
`module agent`. The `require` block (`modernc.org/sqlite v1.50.1` + indirects)
stays as-is; copy `go.sum` verbatim. Engine adds no deps (Section B.1).

### E.3 Internal import paths (`ledger/internal/...` -> `agent/internal/...`)
...
- 75, 77: `"LEDGER_DB_PATH"`, default `"./tmp/ledger.db"` -> `./tmp/agent.db`
...
- 13: `testResourceID = "https://int.ikigenba.com/srv/ledger/mcp"` -> `/srv/agent/mcp`
...
- 58: `"serverInfo": {"name": "Ledger", "version": "1"}` -> `"Agent"`

**internal/mcp/tools.go**:
- 9, 10: comments "ledger_* tool set" / "ledger_whoami"
- 15: `desc("ledger_whoami", "...", ...)` -> `agent_whoami` (tool name + description)
- 56: `case "ledger_whoami":` -> `agent_whoami`

**etc/manifest.env**:
- 1: `APP=ledger` -> `APP=agent`
- 2: `MOUNT=/srv/ledger/` -> `/srv/agent/`
- 4: `PORT=3002` -> `3004`
...
- (rename all `ledger_owner`/`ledger_client`/`ledger_authn_500` nginx vars to `agent_*`)
...
- 5: `APP := ledger` -> `agent`
- 14: `go build ... ./cmd/ledger` -> `./cmd/agent`
...
- 2, 5, 7, 9, 10, 14, 35: comments + paths (`build/ledger`, `/opt/ledger/bin/run`,
  ...
  `/srv/ledger/`)
- 28: `-o "build/${APP}.bin" ./cmd/ledger` -> `./cmd/agent`
...
- 41: `exec /opt/ledger/bin/ledger.bin \`
  (rewrite `LEDGER_*` -> `AGENT_*`, `/srv/ledger/` -> `/srv/agent/`,
  `/opt/ledger` -> `/opt/agent`, `ledger.bin` -> `agent.bin`)
...
- **External deps: NONE — engine is verified stdlib-only** (ikigai-cli go.mod has
  no `require` block). agent adds nothing beyond the chassis `modernc.org/sqlite`.
