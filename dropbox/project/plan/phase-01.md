...
Stand up dropbox's human landing page: a new `dropbox/internal/web` package with a
...
`GET /{$}` through the existing `cmd/dropbox/main.go` `Handlers` hook (nginx is the
gate — Phase 2).

**What gets built (the observable end state):**

- **`dropbox/internal/web/`** — a new package:
...
- **`dropbox/cmd/dropbox/main.go`** — one line added to the **existing**
  `Handlers` hook, beside the `POST /mcp` mount (and the loopback `GET /content` /
  `GET /list` mounts), ungated in-process:
  ...
  mount, the `GET /content` / `GET /list` byte routes, and the
  Service/Producer/Workers (sync engine) wiring are untouched. (If
...
- R-LAND-3C9X — a test constructs `web.LandingHandler("dropbox", v)` and drives a
  `GET /` through it (httptest), asserting status `200`.
- R-LAND-5E2Y — that test asserts the rendered body contains the service name
  passed in (`"dropbox"`).
- R-LAND-7G4Z — a test passes a distinctive version (e.g. `"9.9.9-test"`) and
...
- R-LAND-9J6A — a test asserts the landing response's `Content-Type` is
  `text/html; charset=utf-8`.
- R-ROUT-2B5C — a test registers the handler on an `http.ServeMux` at
  `GET /{$}` (as the composition root wires it) and asserts `GET /` is dispatched
  to the landing handler (`200`, name+version body).
- R-ROUT-4D7E — a test registers a sibling `POST /mcp` on the same mux and asserts
  `{$}` does not shadow it (`POST /mcp` reaches the mcp/stub handler, not the
  landing handler).
- R-ROUT-6F9G — a test asserts a non-root unregistered path (e.g. `GET /nope`) is
  **not** captured by the `{$}` landing pattern (it does not return the landing
  page) — proving exact-match, not subtree.
- R-ASST-3H6J — a test drives `GET /static/tokens.css` through the handler and
  asserts `200` with a CSS content type (`text/css`).
- R-ASST-5K8L — a test asserts the rendered landing HTML references dropbox's own
  embedded `/static/` asset path and contains **no** cross-service/dashboard asset
  URL.
- R-ASST-7M1N — a test drives `GET` for an embedded font (e.g.
