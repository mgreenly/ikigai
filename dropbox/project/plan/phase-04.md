...
dropbox's landing page diverged from the suite-canonical cron page. This phase makes
...
- **`dropbox/internal/web/landing.html`** — make it identical to
  `cron/internal/web/landing.html` except the three substitutions in D6's table:
  the `<title>` suffix (`· dropbox`), the `.eyebrow` text (`Dropbox mirror`), and the
  lead `<p>` (`Dropbox keeps a private local mirror in sync with one Dropbox app folder and publishes change events to the event plane.`). Everything else — the inline `<style>`, the
  `<h1>{{.Service}}</h1>`, the `Service / Version / API` `<dl>` (API value
  `POST /mcp`), and the `/static/tokens.css` link — is copied from cron verbatim.
- **`dropbox/internal/web/static/tokens.css`** — replace with a byte-for-byte copy
  of `cron/internal/web/static/tokens.css`. Leave `static/fonts/` as is.
- **`dropbox/internal/web/web_test.go`** — if any existing assertion pins the prior
  ...
  keep them.
- Touch nothing else: the handler, the routes, `cmd/dropbox/main.go`, and the nginx
...
- `dropbox/internal/web/landing.html` differs from `cron/internal/web/landing.html`
  on only the three lines in D6's table (the `· dropbox` title suffix, the
  `Dropbox mirror` eyebrow, and the `Dropbox keeps a private local mirror in sync with one Dropbox app folder and publishes change events to the event plane.` lead paragraph); all other lines are
  identical.
