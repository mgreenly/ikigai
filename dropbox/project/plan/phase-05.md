...
Dropbox's landing page gains a quiet **top-left Home link** that returns the
viewer to the dashboard landing page — the apex root `/`, which nginx routes to
the dashboard. This is dropbox's own control, built entirely within dropbox; nothing
...
- **`dropbox/internal/web/landing.html`** — add, as the **first child of `<body>`** (before
  `<main>`), the anchor:
...
- **`dropbox/internal/web/web_test.go`** — add a genuinely-asserting test tagged
  `// R-HOME-6P8T` that drives `GET /` through the landing handler and asserts the
...
- R-HOME-6P8T — a test tagged `// R-HOME-6P8T` drives `GET /` through the landing handler and
