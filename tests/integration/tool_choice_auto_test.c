/**
 * @file tool_choice_auto_test.c
 * @brief End-to-end integration test for tool_choice: "auto" behavior
 *
 * Verifies that tool_choice: "auto" works correctly in the full conversation flow:
 * 1. User asks for file search
 * 2. Model decides to use glob tool (with tool_choice: "auto")
...
// ========== Test: Request serialization with tool_choice auto ==========
...
START_TEST(test_request_has_tool_choice_auto) {
    ...
    // Verify tool_choice field exists and is "auto"
...
/**
 * Test the complete flow with tool_choice: "auto":
 * 1. User message: "Find all C files in src/"
 * 2. Model responds with glob tool call (simulated)
 * 3. Tool executes and returns results
 * 4. Model responds with formatted results (simulated)
 * 5. All messages persisted to database correctly
 */
START_TEST(test_tool_choice_auto_end_to_end)
{
    SKIP_IF_NO_DB();
    ...
    const char *tool_call_id = "call_glob_auto_123";
...
/**
 * Verify that ik_tool_choice_auto() returns the correct mode.
 */
START_TEST(test_tool_choice_auto_constructor)
{
    ik_tool_choice_t choice = ik_tool_choice_auto();
...
static Suite *tool_choice_auto_suite(void)
{
    Suite *s = suite_create("Tool Choice Auto");

    TCase *tc_core = tcase_create("Core");
    tcase_add_unchecked_fixture(tc_core, suite_setup, suite_teardown);
    tcase_add_checked_fixture(tc_core, test_setup, test_teardown);
    tcase_add_test(tc_core, test_request_has_tool_choice_auto);
    tcase_add_test(tc_core, test_tool_choice_auto_end_to_end);
    tcase_add_test(tc_core, test_tool_choice_auto_constructor);
...
int main(void)
{
    Suite *s = tool_choice_auto_suite();
