/**
 * @file tool_choice_specific_test.c
 * @brief End-to-end integration test for tool_choice: specific tool behavior
 *
 * Verifies that tool_choice: {"type": "function", "function": {"name": "glob"}} works correctly:
...
// ========== Test: Request serialization with tool_choice specific ==========

/**
 * Verify that ik_openai_serialize_request includes
...
START_TEST(test_request_has_tool_choice_specific) {
    ...
    // Verify tool_choice field exists and is an OBJECT (not string)
    yyjson_val *tool_choice = yyjson_obj_get(root, "tool_choice");
    ck_assert_ptr_nonnull(tool_choice);
    ck_assert(yyjson_is_obj(tool_choice));  // CRITICAL: must be object, not string

    // Verify type field is "function"
    yyjson_val *type_val = yyjson_obj_get(tool_choice, "type");
...
/**
 * Test the complete flow with tool_choice: {"type": "function", "function": {"name": "glob"}}:
 * 1. User message: "Find all C files in src/"
 * 2. Model must call glob tool (simulated - forced by tool_choice)
 * 3. Tool executes and returns results
 * 4. Model responds with formatted results (simulated)
 * 5. All messages persisted to database correctly
 */
START_TEST(test_tool_choice_specific_end_to_end)
{
    SKIP_IF_NO_DB();
    ...
    // Step 2: Model MUST call glob tool (tool_choice was specific "glob")
    // In real flow, this would come from OpenAI API with finish_reason="tool_calls"
    // Model cannot choose a different tool or respond with text only
    const char *tool_call_id = "call_glob_specific_123";
    ...
    // Verify tool_call message has correct structure and is glob
    ...
    // CRITICAL: Verify the tool called is exactly "glob" (not any other tool)
    yyjson_val *name_val = yyjson_obj_get(function, "name");
    ck_assert_ptr_nonnull(name_val);
    ck_assert_str_eq(yyjson_get_str(name_val), "glob");
    ...
    // (This distinguishes specific mode from none mode)
...
/**
 * Verify that ik_tool_choice_specific() returns the correct mode and tool_name.
 */
START_TEST(test_tool_choice_specific_constructor)
{
    TALLOC_CTX *ctx = talloc_new(NULL);
...
/**
 * Verify that ik_tool_choice_specific() works with different tool names.
 */
START_TEST(test_tool_choice_specific_different_tools)
{
    TALLOC_CTX *ctx = talloc_new(NULL);

    // Test with grep
    ik_tool_choice_t grep_choice = ik_tool_choice_specific(ctx, "grep");
    ck_assert_int_eq(grep_choice.mode, IK_TOOL_CHOICE_SPECIFIC);
    ck_assert_str_eq(grep_choice.tool_name, "grep");

    // Test with file_read
    ik_tool_choice_t read_choice = ik_tool_choice_specific(ctx, "file_read");
    ck_assert_int_eq(read_choice.mode, IK_TOOL_CHOICE_SPECIFIC);
    ck_assert_str_eq(read_choice.tool_name, "file_read");

    // Test with bash
    ik_tool_choice_t bash_choice = ik_tool_choice_specific(ctx, "bash");
    ck_assert_int_eq(bash_choice.mode, IK_TOOL_CHOICE_SPECIFIC);
    ck_assert_str_eq(bash_choice.tool_name, "bash");
...
static Suite *tool_choice_specific_suite(void)
{
    Suite *s = suite_create("Tool Choice Specific");

    TCase *tc_core = tcase_create("Core");
    tcase_add_unchecked_fixture(tc_core, suite_setup, suite_teardown);
    tcase_add_checked_fixture(tc_core, test_setup, test_teardown);
    tcase_add_test(tc_core, test_request_has_tool_choice_specific);
    tcase_add_test(tc_core, test_tool_choice_specific_end_to_end);
    tcase_add_test(tc_core, test_tool_choice_specific_constructor);
    tcase_add_test(tc_core, test_tool_choice_specific_different_tools);
...
int main(void)
{
    Suite *s = tool_choice_specific_suite();
