...
{
    unlink(".ikigai/history"); rmdir(".ikigai");
}
...
static void type_str(ik_repl_ctx_t *repl, const char *s)
{
    ...
    for (size_t i = 0; s[i]; i++) {
        a.codepoint = (uint32_t)s[i]; ik_repl_process_action(repl, &a);
    }
}

static void __attribute__((unused)) press_tab(ik_repl_ctx_t *r)
{
    ik_input_action_t a = {.type = IK_INPUT_TAB}; ik_repl_process_action(r, &a);
}

static void __attribute__((unused)) press_esc(ik_repl_ctx_t *r)
{
    ik_input_action_t a = {.type = IK_INPUT_ESCAPE}; ik_repl_process_action(r, &a);
}
