/**
 * @file provider_switching_test.c
 * @brief Integration tests for provider switching and fork inheritance
 *
 * Tests: 14 total
 * - Provider switching (5): inference, fields, invalidation, system prompt, DB update
 * - Thinking level (4): enum values, supports_thinking, preservation, change
 * - Fork inheritance (5): inherit, model override, thinking override, cross-provider, DB hierarchy
 */
#include <check.h>
#include <curl/curl.h>
#include <fcntl.h>
...
CURLMcode curl_multi_add_handle_(CURLM *m, CURL *e) { (void)m; (void)e; mock_running_handles = 1; return CURLM_OK; }
...
static void reset_mock_state(void) { mock_perform_calls = 0; mock_running_handles = 0; }
...
static Suite *provider_switching_suite(void) {
    Suite *s = suite_create("Provider Switching Integration");
...
int main(void) {
    Suite *s = provider_switching_suite();
