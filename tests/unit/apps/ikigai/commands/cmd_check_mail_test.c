/**
 * @file cmd_check_mail_test.c
 * @brief Unit tests for /check-mail command
 */
...
START_TEST(test_check_mail_displays_summary) {
    ...
    res = ik_db_mail_mark_read(db, msg1->id);
    ck_assert(is_ok(&res));
...
START_TEST(test_check_mail_only_current_agent) {
    ...
    other->uuid = talloc_strdup(other, "other-uuid-pqr");
    other->name = NULL;
    other->parent_uuid = NULL;
    other->created_at = 1234567896;
...
static Suite *check_mail_suite(void)
{
    ...
    tcase_add_test(tc, test_check_mail_empty_inbox);
    tcase_add_test(tc, test_check_mail_displays_summary);
    tcase_add_test(tc, test_check_mail_unread_marker);
    tcase_add_test(tc, test_check_mail_message_preview);
    tcase_add_test(tc, test_check_mail_relative_timestamp);
    tcase_add_test(tc, test_check_mail_only_current_agent);
...
int main(void)
{
    ...
    Suite *s = check_mail_suite();
    SRunner *sr = srunner_create(s);
    srunner_set_xml(sr, "reports/check/unit/apps/ikigai/commands/cmd_check_mail_test.xml");
