...
static void setup_repl(void)
{
    ...
    // Add a sender agent for the test
...
// Note: Lines 443, 450, 487, 502 are scrollback_append_line errors
// which are OUT_OF_MEMORY errors from talloc and very difficult to test

static Suite *filter_mail_db_errors_suite(void)
{
    ...
    tcase_add_test(tc, test_filter_mail_db_inbox_filtered_error);
...
int main(void)
{
    ...
    srunner_set_xml(sr, "reports/check/unit/apps/ikigai/commands/cmd_filter_mail_db_errors_test.xml");
