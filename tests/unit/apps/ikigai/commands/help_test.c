...
START_TEST(test_help_lists_mark) {
    ...
    // Line 11 should be /mark (alphabetically: ..., load, mark, model, ...)
    const char *line = NULL;
    size_t length = 0;
...
START_TEST(test_help_lists_rewind) {
    ...
    // Line 21 should be /rewind (alphabetically: ..., pwrite, reap, refresh, rewind, ...)
    const char *line = NULL;
    size_t length = 0;
    res = ik_scrollback_get_line_text(repl->current->scrollback, 21, &line, &length);
    ck_assert(is_ok(&res));
    ck_assert_ptr_nonnull(line);
...
START_TEST(test_help_lists_model) {
    ...
    // Line 12 should be /model (alphabetically: ..., mark, model, pin, ...)
    const char *line = NULL;
    size_t length = 0;
...
START_TEST(test_help_lists_exit) {
    ...
    // Line 6 should be /exit (alphabetically: ..., context, exit, fork, ...)
    const char *line = NULL;
    size_t length = 0;
