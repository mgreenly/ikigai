...
static ik_result_t helper_success(TALLOC_CTX *ctx)
{
...
static ik_result_t helper_error(TALLOC_CTX *ctx)
{
    return ERR(ctx, INVALID_ARG, "Test error message");
}

// Helper function using CHECK macro
static ik_result_t helper_propagate(TALLOC_CTX *ctx, bool should_fail)
{
    ik_result_t res;
