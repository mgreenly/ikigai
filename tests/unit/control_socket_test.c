...
START_TEST(test_listen_ready_false)
{
    ...
    bool ready = ik_control_socket_listen_ready(ctl, &read_fds);
    ck_assert(!ready);
...
START_TEST(test_accept_connection)
{
    ...
    int32_t conn_result = connect(client_fd, (struct sockaddr *)&addr, sizeof(addr));
    ck_assert_int_eq(conn_result, 0);
...
START_TEST(test_destroy_with_client)
{
    ...
    // Destroy with client connected - should close both fds
    close(client_fd);
    ik_control_socket_destroy(ctl);
    rmdir(tmpdir);
    talloc_free(ctx);
