/**
 * @file event_handlers_curl_events_test.c
 * @brief Coverage tests for ik_repl_handle_curl_events in repl_event_handlers.c
 *
 * Tests for Line 325: ik_repl_handle_curl_events with current not in agents array
 */
...
static res_t mock_fdset(void *pctx, fd_set *read_fds, fd_set *write_fds, fd_set *exc_fds, int *max_fd)
{
    mock_provider_ctx_t *mock = (mock_provider_ctx_t *)pctx;
    return ik_openai_multi_fdset(mock->multi, read_fds, write_fds, exc_fds, max_fd);
}

static res_t mock_perform(void *pctx, int *running_handles)
{
    mock_provider_ctx_t *mock = (mock_provider_ctx_t *)pctx;
    return ik_openai_multi_perform(mock->multi, running_handles);
}

static res_t mock_timeout(void *pctx, long *timeout_ms)
{
    mock_provider_ctx_t *mock = (mock_provider_ctx_t *)pctx;
    return ik_openai_multi_timeout(mock->multi, timeout_ms);
}

static void mock_info_read(void *pctx, ik_logger_t *logger)
{
    mock_provider_ctx_t *mock = (mock_provider_ctx_t *)pctx;
 ...
 * Test suite
 */
static Suite *event_handlers_curl_events_suite(void)
{
    Suite *s = suite_create("Event Handlers CURL Events");
...
int main(void)
{
    Suite *s = event_handlers_curl_events_suite();
