 ...
 *
 * These tests verify that scrollback, separator, and workspace scroll together
 * as a single unified document, rather than separator/workspace being "sticky"
 * at the bottom.
 */
...
START_TEST(test_separator_scrolls_offscreen)
{
    ...
    // Create workspace with single line
    ...
    // Document height = 50 (scrollback) + 1 (separator) + 1 (workspace) = 52 lines
    // Terminal shows 10 lines
    // When offset = 42, we're showing lines 0-9 of scrollback
    // Separator is at line 50, workspace at line 51 - both OFF SCREEN
    ...
    // Verify workspace content 'x' does NOT appear
 ...
 *
 * Similar to above, but specifically checks that workspace content is scrolled off.
 */
START_TEST(test_workspace_scrolls_offscreen)
{
    void *ctx = talloc_new(NULL);
    ...
    // Create workspace with distinctive content
    ik_workspace_t *workspace = NULL;
    res_t res = ik_workspace_create(ctx, &workspace);
    ck_assert(is_ok(&res));
    const char *workspace_content = "WORKSPACE_MARKER_TEXT";
    for (size_t i = 0; i < strlen(workspace_content); i++) {
        res = ik_workspace_insert_codepoint(workspace, (uint32_t)(unsigned char)workspace_content[i]);
        ck_assert(is_ok(&res));
    }
    ik_workspace_ensure_layout(workspace, 80);
    ...
    for (int32_t i = 0; i < 50; i++) {
        char buf[32];
        snprintf(buf, sizeof(buf), "line%" PRId32, i);  // Intentionally different from workspace
        res = ik_scrollback_append_line(scrollback, buf, strlen(buf));
        ck_assert(is_ok(&res));
    }
    ...
    // Scroll to middle of scrollback (workspace is off-screen)
    ...
    // Verify workspace content does NOT appear
...
START_TEST(test_scrollback_adjacent_to_separator)
{
    ...
    // Create terminal context (20 rows - enough to show some scrollback + separator + workspace)
    ik_term_ctx_t *term = talloc_zero(ctx, ik_term_ctx_t);
    term->screen_rows = 20;
    term->screen_cols = 80;
...
static Suite *repl_document_scrolling_suite(void)
{
    ...
    tcase_add_test(tc_scrolling, test_workspace_scrolls_offscreen);
