...
END_TEST START_TEST(test_config_auto_create_defaults)
{
    ...
    ck_assert_int_eq(cfg->openai_max_tokens, 4096);
...
END_TEST START_TEST(test_config_memory_cleanup)
{
    ...
    ck_assert_ptr_nonnull(f);
    fprintf(f,
            "{\"openai_api_key\": \"test_key\", \"openai_model\": \"gpt-4-turbo\", \"openai_temperature\": 0.7, \"openai_max_tokens\": 4096, \"openai_system_message\": null, \"listen_address\": \"127.0.0.1\", \"listen_port\": 8080}");
    fclose(f);
