...
END_TEST START_TEST(test_config_missing_field_listen_address)
{
    ...
    ck_assert_ptr_nonnull(f);
    fprintf(f,
            "{\"openai_api_key\": \"test\", \"openai_model\": \"gpt-5-mini\", \"openai_temperature\": 0.7, \"openai_max_tokens\": 4096, \"openai_system_message\": null, \"listen_port\": 1984}");
    fclose(f);
...
END_TEST START_TEST(test_config_missing_field_listen_port)
{
    ...
    ck_assert_ptr_nonnull(f);
    fprintf(f,
            "{\"openai_api_key\": \"test\", \"openai_model\": \"gpt-5-mini\", \"openai_temperature\": 0.7, \"openai_max_tokens\": 4096, \"openai_system_message\": null, \"listen_address\": \"127.0.0.1\"}");
    fclose(f);
...
END_TEST START_TEST(test_config_missing_field_openai_max_tokens)
{
    TALLOC_CTX *ctx = talloc_new(NULL);
    ck_assert_ptr_nonnull(ctx);
...
static Suite *config_validation_missing_suite(void)
{
    ...
    tcase_add_test(tc_core, test_config_missing_field_openai_max_tokens);
