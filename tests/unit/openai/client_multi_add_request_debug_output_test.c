...
START_TEST(test_multi_add_request_with_debug_output_file) {
    ...
    /* Create debug output - use stderr for testing (always available) */
    FILE *debug_output = stderr;

    /* Add request with debug output - should succeed without crashing */
    res_t add_res = ik_openai_multi_add_request(multi, cfg, conv, NULL, NULL, NULL, NULL, debug_output, false);
...
END_TEST START_TEST(test_multi_add_request_no_debug_output)
{
    ...
    /* Add request with NULL debug output - should not crash */
    res_t add_res = ik_openai_multi_add_request(multi, cfg, conv, NULL, NULL, NULL, NULL, NULL, false);
    ck_assert(!add_res.is_err);
}
