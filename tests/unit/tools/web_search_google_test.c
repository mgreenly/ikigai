...
START_TEST(test_schema_flag) {
    ...
    ck_assert(strstr(desc_str, "Google search") != NULL);
...
START_TEST(test_missing_both_credentials) {
    ...
    ck_assert(strstr(content_str, "Need api_key") != NULL);
...
START_TEST(test_large_json_input) {
    ...
    char large_input[4096];
    char *p = large_input;
    p += snprintf(p, sizeof(large_input) - (size_t)(p - large_input), "{\"query\":\"");
    for (int32_t i = 0; i < 350; i++) {
        p += snprintf(p, sizeof(large_input) - (size_t)(p - large_input), "search ");
    }
    snprintf(p, sizeof(large_input) - (size_t)(p - large_input), "\"}");

    int32_t exit_code = -1;
    run_tool_with_output(large_input, &exit_code);
...
START_TEST(test_exact_buffer_size) {
    ...
    char exact_input[2049];
    char *p = exact_input;
    p += snprintf(p, sizeof(exact_input) - (size_t)(p - exact_input), "{\"query\":\"");
    while ((size_t)(p - exact_input) < 2037) {
        p += snprintf(p, sizeof(exact_input) - (size_t)(p - exact_input), "x");
    }
    snprintf(p, sizeof(exact_input) - (size_t)(p - exact_input), "\"}");

    int32_t exit_code = -1;
    run_tool_with_output(exact_input, &exit_code);
