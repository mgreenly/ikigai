...
START_TEST(test_missing_query) {
    ...
    int32_t exit_code = -1;
    run_tool_with_output("{\"num\": 5}", &exit_code);
...
START_TEST(test_invalid_query_type) {
    ...
    int32_t exit_code = -1;
    run_tool_with_output("{\"query\": 123}", &exit_code);
...
START_TEST(test_large_input) {
    ...
    char *large_input = talloc_asprintf(test_ctx, "{\"query\": \"test search");
    for (int32_t i = 0; i < 500; i++) {
        large_input = talloc_asprintf_append(large_input, " word");
    }
    large_input = talloc_asprintf_append(large_input, "\"}");

    int32_t exit_code = -1;
    char *output = run_tool_with_output(large_input, &exit_code);
...
START_TEST(test_multi_domain_invalid_domain) {
    setenv("GOOGLE_SEARCH_API_KEY", "test-api-key", 1);
    setenv("GOOGLE_SEARCH_ENGINE_ID", "test-engine-id", 1);

    // Test with invalid domain (number instead of string) - hits line 65
    int32_t exit_code = -1;
    char *output = run_tool_with_output("{\"query\":\"test\",\"allowed_domains\":[123,\"example.com\"]}", &exit_code);
...
START_TEST(test_multi_domain_zero_results) {
    setenv("GOOGLE_SEARCH_API_KEY", "test-api-key", 1);
    setenv("GOOGLE_SEARCH_ENGINE_ID", "test-engine-id", 1);

    // Test with num < allowed_count, causing some domains to get 0 results - hits line 72
    int32_t exit_code = -1;
    char *output =
        run_tool_with_output("{\"query\":\"test\",\"num\":1,\"allowed_domains\":[\"a.com\",\"b.com\",\"c.com\"]}",
                             &exit_code);
...
static Suite *web_search_google_suite(void)
{
    ...
    tcase_add_test(tc_core, test_multi_domain_invalid_domain);
    tcase_add_test(tc_core, test_multi_domain_zero_results);
...
int main(void)
{
    ...
    srunner_set_xml(sr, "reports/check/unit/tools/web_search_google_test.xml");
