...
START_TEST(test_cursor_up_down_column_preservation) {
     ...
     */
    workspace->cursor_byte_offset = 16;
    char *text;
    size_t text_len;
    ik_workspace_get_text(workspace, &text, &text_len);
    ik_cursor_set_position(workspace->cursor, text, text_len, 16);
     ...
     */
    res = ik_workspace_cursor_down(workspace);
    ck_assert(is_ok(&res));
...
START_TEST(test_multiple_vertical_movements)
{
     ...
     */
    workspace->cursor_byte_offset = 38;
    char *text;
    size_t text_len;
    ik_workspace_get_text(workspace, &text, &text_len);
    ik_cursor_set_position(workspace->cursor, text, text_len, 38);
