...
START_TEST(test_serialize_reasoning_none) {
    ...
    // For o1/o3 models, NONE maps to "low" effort
    yyjson_val *reasoning = yyjson_obj_get(root, "reasoning");
    ck_assert_ptr_nonnull(reasoning);
    yyjson_val *effort = yyjson_obj_get(reasoning, "effort");
    ck_assert_ptr_nonnull(effort);
...
START_TEST(test_serialize_gpt5_reasoning_none) {
    ...
    // For gpt-5 family, NONE maps to NULL (no reasoning field)
    ik_request_set_thinking(req, IK_THINKING_NONE, false);
    ik_request_add_message(req, IK_ROLE_USER, "Test");
...
START_TEST(test_serialize_gpt5_reasoning_low) {
    ik_request_t *req = NULL;
    res_t create_result = ik_request_create(test_ctx, "gpt-5", &req);
    ck_assert(!is_err(&create_result));
    ...
    // For gpt-5 with LOW, reasoning field should be present with effort="low"
    yyjson_val *reasoning = yyjson_obj_get(root, "reasoning");
    ck_assert_ptr_nonnull(reasoning);
    yyjson_val *effort = yyjson_obj_get(reasoning, "effort");
    ck_assert_ptr_nonnull(effort);
...
START_TEST(test_serialize_gpt5_pro_reasoning_none) {
    ik_request_t *req = NULL;
    res_t create_result = ik_request_create(test_ctx, "gpt-5-pro", &req);
    ck_assert(!is_err(&create_result));

    // For gpt-5-pro, all levels map to "high"
    ik_request_set_thinking(req, IK_THINKING_NONE, false);
    ik_request_add_message(req, IK_ROLE_USER, "Test");
    ...
    // For gpt-5-pro with NONE, reasoning field should be present with effort="high"
    yyjson_val *reasoning = yyjson_obj_get(root, "reasoning");
    ck_assert_ptr_nonnull(reasoning);
    yyjson_val *effort = yyjson_obj_get(reasoning, "effort");
    ck_assert_ptr_nonnull(effort);
...
static Suite *request_responses_reasoning_tools_suite(void)
{
    ...
    tcase_add_test(tc_reasoning, test_serialize_gpt5_reasoning_low);
    tcase_add_test(tc_reasoning, test_serialize_gpt5_pro_reasoning_none);
...
int main(void)
{
    ...
    srunner_set_xml(sr, "reports/check/unit/providers/openai/request_responses_reasoning_tools_test.xml");
