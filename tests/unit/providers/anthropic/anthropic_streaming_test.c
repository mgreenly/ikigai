/**
 * @file test_anthropic_streaming.c
 * @brief Unit tests for Anthropic streaming through provider vtable
 *
 * Tests async streaming using VCR fixtures in JSONL format.
 * Verifies fdset/perform/info_read pattern and SSE event processing.
 */
...
static Suite *anthropic_streaming_suite(void)
{
    Suite *s = suite_create("Anthropic Streaming");
...
int main(void)
{
    Suite *s = anthropic_streaming_suite();
