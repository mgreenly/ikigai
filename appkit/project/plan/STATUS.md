...
Phase 12 ⬜ realizes D8 (revised) — structured results in `appkit/mcp`: protocol `2025-06-18`, `Tool.OutputSchema` surfaced in `tools/list`, `StructuredResult` (deletes `JSONResult`), typed `ErrorCode` + re-signed `ErrorResult`, handler faults → `-32603`; ⛔ coordinated cutover — services convert in their own adoption phases, suite deploys together; covers R-WPNN-6Q9E, R-WQVJ-KI03, R-WTBC-C1HH, R-WUJ8-PT86, R-WVR5-3KYV
Phase 13 ⬜ realizes D9 (revised) — structured standard tools: `health`/`reflection` return `structuredContent` with declared `outputSchema`s, unknown kind → coded `validation` error; covers R-WWZ1-HCPK, R-WY6X-V4G9, R-WZEU-8W6Y
Phase 14 ⬜ realizes D12 — loopback-only route class in `appkit/server`: `LoopbackOnly` (X-Forwarded-Proto-only predicate), `Router.HandleLoopback`, `/feed` mount wrapped; ⛔ eventplane's inline guard deletion + service guard swaps land in their own specs; covers R-X0MQ-MNXN, R-X1UN-0FOC, R-X32J-E7F1, R-X4AF-RZ5Q
