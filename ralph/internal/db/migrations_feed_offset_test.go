...
// TestFeedOffsetMigrationMatchesLibraryDDL guards that ralph's 004_feed_offset.sql
// applies the eventplane library's feed_offset DDL (consumer.SchemaSQL) verbatim.
// ralph is a CONSUMER of cron's /feed; if the migration drifts from the library
// constant, ralph's offset store is no longer the schema the engine reads and
