...
Crm's landing page gains a quiet **top-left Home link** that returns the
viewer to the dashboard landing page — the apex root `/`, which nginx routes to
the dashboard. This is crm's own control, built entirely within crm; nothing
...
- **`crm/internal/web/landing.html`** — add, as the **first child of `<main>`** (inside
...
- **`crm/internal/web/web_test.go`** — add a genuinely-asserting test tagged
  `// R-HOME-3L5Q` that drives `GET /` through the landing handler and asserts the
...
- R-HOME-3L5Q — a test tagged `// R-HOME-3L5Q` drives `GET /` through the landing handler and
