...
Stand up crm's human landing page: a new `crm/internal/web` package with a
...
`GET /{$}` through the existing `cmd/crm/main.go` `Handlers` hook (nginx is the
gate — Phase 2).

**What gets built (the observable end state):**

- **`crm/internal/web/`** — a new package:
...
- **`crm/cmd/crm/main.go`** — one line added to the **existing** `Handlers` hook,
...
- R-LAND-2K7P — a test constructs `web.LandingHandler("crm", v)` and drives a
  `GET /` through it (httptest), asserting status `200`.
- R-LAND-4M9Q — that test asserts the rendered body contains the service name
  passed in (`"crm"`).
- R-LAND-6N3R — a test passes a distinctive version (e.g. `"9.9.9-test"`) and
...
- R-LAND-8P5S — a test asserts the landing response's `Content-Type` is
  `text/html; charset=utf-8`.
- R-ROUT-3T2V — a test registers the handler on an `http.ServeMux` at
  `GET /{$}` (as the composition root wires it) and asserts `GET /` is dispatched
  to the landing handler (`200`, name+version body).
- R-ROUT-5W4X — a test registers a sibling `POST /mcp` on the same mux and asserts
  `{$}` does not shadow it (`POST /mcp` reaches the mcp/stub handler, not the
  landing handler).
- R-ROUT-7Y6Z — a test asserts a non-root unregistered path (e.g. `GET /nope`) is
  **not** captured by the `{$}` landing pattern (it does not return the landing
  page) — proving exact-match, not subtree.
- R-ASST-2B8C — a test drives `GET /static/tokens.css` through the handler and
  asserts `200` with a CSS content type (`text/css`).
- R-ASST-4D1E — a test asserts the rendered landing HTML references crm's own
  embedded `/static/` asset path and contains **no** cross-service/dashboard asset
  URL.
- R-ASST-6F3G — a test drives `GET` for an embedded font (e.g.
