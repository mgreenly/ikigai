...
- `src/wrapper.c` - External library wrappers as link seams for testing
...
- `src/byte_array.c`, `src/line_array.c` - Typed wrappers (uint8_t, char*) over generic array
...
- `src/input_buffer/cursor_pp.c`, `src/input_buffer/pp.c` - Debug pretty-print
...
- `src/commands_basic.c` - /clear, /help, /model, /system, /debug
...
- `src/providers/google/response.c` - Tool call ID generation, finish reason mapping
