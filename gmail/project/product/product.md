# gmail — Product (landing page)

**Authority: intent.** This document owns *why* gmail serves a web landing page,
...
> for gmail — the landing page. gmail's existing Gmail-connector domain (the
> normal-mailbox MCP tool surface, the History-API poll daemon, and the `mail.*`
> event-plane producer half) is owned by gmail's service doctrine (the
> `cmd/gmail/main.go` package comment and the connector decisions); it is
> untouched here. This mirrors the suite's uniform v1 landing-page starting
> point, the same one the simple services adopt.
...
the loopback `/feed`. A human who opened `<account>.ikigenba.com/srv/gmail/` in a
...
services declared no human web surface, and that statement is now being
...
The gmail landing page is the **human front door** to the gmail service under
`/srv/gmail/`. For v1 it is intentionally minimal: a single Carbon-styled card
...
embedded template + embedded design assets) that every later gmail web page grows
from.

## Users

- **A logged-in dashboard user, in a browser.** Any human authenticated to this
  box's dashboard who navigates to `/srv/gmail/`. They see the service name and
...
- **The operator, confirming a deploy.** Opens the mount root after a deploy or
  rollback to confirm gmail is up and which version is live — a browser-visible
...
The gmail landing page does this and only this:

- **Serve one landing page at the mount root** — a `GET` of the bare `/srv/gmail/`
...
- **Show the service name and version** — the page displays the service name
  (`gmail`) and the running version, taken from the values the chassis already
...
- **Carry its own design assets** — gmail embeds its **own** copy of the Carbon
...
- **Gate humans by the dashboard session cookie** — the page (and any future gmail
...
design); expose any mailbox, message, thread, or `mail.*` event on the page; add
or change any MCP tool; serve any interactive control, form, or write action;
alter the bearer-gated `/mcp`, the PRM well-known, `/health`, or the loopback
`/feed`; touch the History-API poll daemon or the event producer; or share a
landing handler with any other service. Later gmail-specific web pages are **out
of scope** for this work — this establishes only the uniform v1 page and the seam
they will grow from.
...
- **The landing page lives at the mount root only.** A human reaches it at
  `<account>.ikigenba.com/srv/gmail/`; the service answers it at its exact root
...
- **Each app owns its own landing page.** There is no shared landing handler;
  gmail's page code, template, and embedded assets live under `gmail/`.
- **The visual system is Carbon.** `design/carbon.md` (rules) + `design/tokens.css`
  (tokens) + `design/example.html` (reference) are the source of truth; gmail
  embeds its own copy of the tokens and fonts.

## What we promise (user-facing behavior)

- **A logged-in human who opens `/srv/gmail/` sees a real page** — the gmail
...
- **A browser that is not logged in is refused** — an unauthenticated browser
  hitting `/srv/gmail/` gets `401`, because the page is gated by the dashboard
  session cookie.
...
- As a logged-in dashboard user I open `<account>.ikigenba.com/srv/gmail/` and see
  a Carbon-styled page showing the service name `gmail` and the running version.
- As a browser with no dashboard session I open `/srv/gmail/` and am refused with
...
- Opening `/srv/gmail/feed` from nginx still returns `404`, and `/health` still
