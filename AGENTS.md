...
| **ralph** | Path-routed service `/srv/ralph/` — runs sandboxed Claude agent sessions on the owner's behalf, exposed as MCP (design-stage). |
...
| **agentkit** | Shared Go **library** for the agent-backed services (ralph/dropbox/wiki). |
...
The seven deployable apps are **dashboard, crm, ledger, notify, dropbox, ralph,
