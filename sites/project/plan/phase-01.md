...
Stand up sites's standardized human landing card: a new `sites/internal/web`
package with a Carbon-styled handler showing the service name + running version,
its own embedded `tokens.css` + woff2 fonts, mounted ungated in-process at the
exact root `GET /{$}` through the existing `cmd/sites/main.go` `Handlers` hook
(nginx is the gate — Phase 2). The page is rendered in-process (so it reflects the
live version), distinct from the public/private static tiers sites serves off
disk.

**What gets built (the observable end state):**

- **`sites/internal/web/`** — a new package:
...
- **`sites/cmd/sites/main.go`** — one line added to the **existing** `Handlers`
  ...
  This is incremental growth of the wiring file, not a rewrite. The existing
  layout/store/mirror-client wiring and the `POST /mcp` mount are untouched. (If
  `rt.HandleFunc` is not the exact appkit Router method name, use the Router's
...
- R-LAND-3C9K — a test constructs `web.LandingHandler("sites", v)` and drives a
  `GET /` through it (httptest), asserting status `200`.
- R-LAND-5E2M — that test asserts the rendered body contains the service name
  passed in (`"sites"`).
- R-LAND-7G4P — a test passes a distinctive version (e.g. `"9.9.9-test"`) and
...
- R-LAND-9J6R — a test asserts the landing response's `Content-Type` is
  `text/html; charset=utf-8`.
- R-ROUT-4Q8B — a test registers the handler on an `http.ServeMux` at
  `GET /{$}` (as the composition root wires it) and asserts `GET /` is dispatched
  to the landing handler (`200`, name+version body).
- R-ROUT-6S1D — a test registers a sibling `POST /mcp` on the same mux and asserts
  `{$}` does not shadow it (`POST /mcp` reaches the mcp/stub handler, not the
  landing handler).
- R-ROUT-8U3F — a test asserts a non-root unregistered path (e.g. `GET /nope`) is
  **not** captured by the `{$}` landing pattern (it does not return the landing
  page) — proving exact-match, not subtree.
- R-ASST-3H7N — a test drives `GET /static/tokens.css` through the handler and
  asserts `200` with a CSS content type (`text/css`).
- R-ASST-5K9Q — a test asserts the rendered landing HTML references sites's own
  embedded `/static/` asset path and contains **no** cross-service/dashboard asset
  URL.
- R-ASST-7M2S — a test drives `GET` for an embedded font (e.g.
