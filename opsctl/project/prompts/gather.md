...
write **only** `project/prompts/brief.md` (its contract region), run no build, no
...
3. **Author a fresh brief.** Only now open the big docs, and only the slice you
   ...
   - Write `project/prompts/brief.md` to the schema below with an **empty**
     feedback region.
...
surrounding prose) so `grep -oE 'R-[A-Z0-9]{4}-[A-Z0-9]{4}' project/prompts/brief.md`
