...
// ralph is the upstream source ralph presents on its outbox envelopes (§8.3) and
// the key for notify's SECOND feed_offset row (the crm loop owns the "crm" row,
// this loop owns the "ralph" row — independent cursors, never shared).
const ralphSource = "ralph"

// Outcome event types ralph emits on its /feed (event-triggering decisions §3,
...
// runOutcome is the slice of ralph's run.succeeded / run.failed payload
...
// RalphSubscriptions are notify's two declared in-edges on ralph's feed
// (event-triggering decisions §4): run.succeeded and run.failed. As with the crm
// Subscription, these are the ONE source of truth — the ralph consumer Handler
...
func RalphSubscriptions() []consumer.Subscription {
	return []consumer.Subscription{
		{
			Source:      ralphSource,
			Filter:      eventRunSucceeded,
			Description: "fires a best-effort ntfy.sh push (Title \"Run succeeded\", body = session_name) when a ralph run finishes successfully",
		},
		{
			Source:      ralphSource,
			Filter:      eventRunFailed,
			Description: "fires a best-effort ntfy.sh push (Title \"Run failed\", body = session_name + error) when a ralph run terminates in failure",
		},
	}
}

// RalphHandler returns the consumer.Handler notify hands to the ralph consumer
...
// waiting; a slow or dead ntfy therefore never stalls ralph's feed (best-effort —
...
func RalphHandler(c *Client, logger *slog.Logger) consumer.Handler {
	if logger == nil {
		logger = slog.Default()
	}
