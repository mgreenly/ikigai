...
Notify's landing page gains a quiet **top-left Home link** that returns the
viewer to the dashboard landing page — the apex root `/`, which nginx routes to
the dashboard. This is notify's own control, built entirely within notify; nothing
...
- **`notify/internal/web/landing.html`** — add, as the **first child of `<main>`** (inside
...
- **`notify/internal/web/web_test.go`** — add a genuinely-asserting test tagged
  `// R-HOME-5N7S` that drives `GET /` through the landing handler and asserts the
...
- R-HOME-5N7S — a test tagged `// R-HOME-5N7S` drives `GET /` through the landing handler and
