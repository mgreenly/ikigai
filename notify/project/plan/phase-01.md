...
Stand up notify's human landing page: a new `notify/internal/web` package with a
...
`GET /{$}` through the existing `cmd/notify/main.go` `Handlers` hook (nginx is the
gate — Phase 2).

**What gets built (the observable end state):**

- **`notify/internal/web/`** — a new package:
...
- **`notify/cmd/notify/main.go`** — one line added to the **existing** `Handlers`
  ...
  mount, the push-client construction, and the consumer `Workers` wiring are
  untouched. (If `rt.HandleFunc` is not the exact appkit Router method name, use
  the Router's equivalent route-register call for `GET /{$}` — the contract is:
  the bare root, exact-match, served by the landing handler.)
...
- R-LAND-3C8K — a test constructs `web.LandingHandler("notify", v)` and drives a
  `GET /` through it (httptest), asserting status `200`.
- R-LAND-5D1M — that test asserts the rendered body contains the service name
  passed in (`"notify"`).
- R-LAND-7E4N — a test passes a distinctive version (e.g. `"9.9.9-test"`) and
...
- R-LAND-9F6P — a test asserts the landing response's `Content-Type` is
  `text/html; charset=utf-8`.
- R-ROUT-4G2Q — a test registers the handler on an `http.ServeMux` at
  `GET /{$}` (as the composition root wires it) and asserts `GET /` is dispatched
  to the landing handler (`200`, name+version body).
- R-ROUT-6H5R — a test registers a sibling `POST /mcp` on the same mux and asserts
  `{$}` does not shadow it (`POST /mcp` reaches the mcp/stub handler, not the
  landing handler).
- R-ROUT-8J7S — a test asserts a non-root unregistered path (e.g. `GET /nope`) is
  **not** captured by the `{$}` landing pattern (it does not return the landing
  page) — proving exact-match, not subtree.
- R-ASST-3K9T — a test drives `GET /static/tokens.css` through the handler and
  asserts `200` with a CSS content type (`text/css`).
- R-ASST-5L2V — a test asserts the rendered landing HTML references notify's own
  embedded `/static/` asset path and contains **no** cross-service/dashboard asset
  URL.
- R-ASST-7M4W — a test drives `GET` for an embedded font (e.g.
